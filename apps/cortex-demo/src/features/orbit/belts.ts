 ...
 *
 * Теснота на отдалении не разводится алгоритмом, а снимается тем, что подписи
 * гаснут: на общем виде читаются форма и плотность, имена появляются, когда
 * человек приблизился к тому, что его занимает. Внутри черты решения подписи
 * не гаснут никогда — край может молчать, середина говорит всегда.
 */
export function labelsVisible(scale: number, belt: number): boolean {
  if (belt <= 3) return scale >= 0.6;
  return scale >= 0.85;
}
