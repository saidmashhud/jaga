...
func New(base, key, model string) *Client {
	...
	return &Client{
		...
		key:   key,
		model: model,
		// Таймаут щедрый: на этой машине модель отвечает десятками секунд.
		// Именно поэтому разбор идёт отдельно от сохранения записи, а не в
		// том же запросе — человек не должен ждать модель, чтобы записать
		// мысль.
...
Поля:
  ...
  offsetHours  — когда это происходит относительно сейчас, в часах; 0 если срок не назван, отрицательное для прошлого
