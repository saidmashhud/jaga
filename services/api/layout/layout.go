...
// Перечислены все статусы, какие есть в данных. Незнакомый статус не
// проваливается молча в ноль: раньше так вёл себя «working», и поведение
// оказалось верным по совпадению, а не по решению — следующий незнакомый
// встал бы туда же и никто бы не заметил.
...
func UnknownStatuses(nodes []Node) []string {
	...
	var out []string
	for _, n := range nodes {
		if _, ok := attentionZ[n.Status]; !ok && !seen[n.Status] {
			seen[n.Status] = true
